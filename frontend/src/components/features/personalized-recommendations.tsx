...
export function PersonalizedRecommendations() {
  ...
  const [result, setResult] = useState<ProvidePersonalizedRecommendationsOutput | null>(null);
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      const recommendationsResult = await providePersonalizedRecommendations(values);
  ...
  return (
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
      <div className="lg:col-span-1">
        <Card>
          <CardHeader>
            <CardTitle className="font-headline">Get Recommendations</CardTitle>
            <CardDescription>
          ...
          <CardContent>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                <FormField control={form.control} name="industrySegmentation" render={({ field }) => (
                  <FormItem>
                    <FormLabel>Industry</FormLabel>
                    <FormControl><Textarea placeholder="e.g., E-commerce, Retail" {...field} className="h-12" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <FormField control={form.control} name="revenueStreams" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Textarea placeholder="e.g., Online sales, subscription fees" {...field} className="h-20" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <FormField control={form.control} name="costStructures" render={({ field }) => (
                  <FormItem>
                    <FormLabel>Cost Structures</FormLabel>
                    <FormControl><Textarea placeholder="e.g., Cost of goods sold, marketing expenses" {...field} className="h-20" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <FormField control={form.control} name="expenseCategories" render={({ field }) => (
                  <FormItem>
                    <FormLabel>Expense Categories</FormLabel>
                    <FormControl><Textarea placeholder="e.g., Salaries, rent, utilities" {...field} className="h-20" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <FormField control={form.control} name="accountsReceivable" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Textarea placeholder="e.g., Average collection period, outstanding invoices" {...field} className="h-20" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                 <FormField control={form.control} name="accountsPayable" render={({ field }) => (
                  <FormItem>
                    <FormLabel>Accounts Payable</FormLabel>
                    <FormControl><Textarea placeholder="e.g., Payment terms with suppliers" {...field} className="h-20" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
          ...
          <CardContent>
            ...
            {result && (
              <div className="space-y-6">
                <ResultCard title="Actionable Insights" content={result.insights} icon={<Lightbulb className="h-5 w-5 text-yellow-500" />} />
                <ResultCard title="Cost Optimization Strategies" content={result.costOptimizationStrategies} icon={<CircleDollarSign className="h-5 w-5 text-green-500" />} />
                <ResultCard title="Suitable Financial Products" content={result.financialProducts} icon={<Package className="h-5 w-5 text-blue-500" />} />
              </div>
            )}
          </CardContent>
        </Card>
